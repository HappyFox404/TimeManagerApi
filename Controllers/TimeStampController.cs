...
public class TimeStampController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IResult> Get(Guid scheduleId)
    {
        ...
        if (timestamps.Any() == false)
            return StandartResponseAnswer.Ok("Не найдено вреаменных меток");
        
        return StandartResponseAnswer.Ok(timestamps);
    }
    
    [HttpGet("day")]
    public async Task<IResult> GetForDay(DateTime day)
    {
        if (day == default)
            return StandartResponseAnswer.Error("Передана неверная дата дня");
    ...
    [HttpGet("id")]
    public async Task<IResult> GetForId(Guid id)
    {
        var needUserId = await _userService.GetCurrentUserId();
        ...
        if (needTimeStamp == null)
            return StandartResponseAnswer.Ok("Не найдена временная метка");
        return StandartResponseAnswer.Ok(needTimeStamp);
    }

    [HttpPost]
    public async Task<IResult> Post(TimeStampCreateFromSchedule model)
    {
        var needUserId = await _userService.GetCurrentUserId();
        var needSchedule = await _context.Schedules
            .FirstOrDefaultAsync(x => x.Id == model.ScheduleId && x.UserId == needUserId);
        if (needSchedule == null)
            return StandartResponseAnswer.Error("Не найдено расписание для создания временной метки");

        if (String.IsNullOrWhiteSpace(model.Title))
            return StandartResponseAnswer.Error("Поле Заголовок обязательно для заполнения");
        if (model.Title.Length > 60)
            return StandartResponseAnswer.Error("Поле Заголовок не может быть более 60 символов");
        ...
        if (exsistsTimeStamp != null)
            return StandartResponseAnswer.Error("Уже есть временная метка на данный период времени");
    ...
    public async Task<IResult> Post(TimeStampCreateWithSchedule model)
    {
        var needUserId = await _userService.GetCurrentUserId();

        if (model.Day == default)
            return StandartResponseAnswer.Error("Передана неверная дата для расписания");
    ...
    [HttpPatch]
    public async Task<IResult> Patch(TimeStampEdit model)
    {
        var needUserId = await _userService.GetCurrentUserId();
    ...
    [HttpDelete]
    public async Task<IResult> Delete(Guid timeStampId)
    {
        var needUserId = await _userService.GetCurrentUserId();
        var needTimeStamp = await _context.TimeStamps
            .Include(x => x.Schedule)
